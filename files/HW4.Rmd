...
|  The main objective of this study is to forecast 2 week period by decomposition of the time series. We have the electricity consumption series from the 1st of January, 2017 till 7th of January, 2021. The data is provided by [EPİAŞ](https://seffaflik.epias.com.tr/transparency/). Daily consumption series is not stationary due several reasonas such as religious holidays, special days and other types of seasonality. We are required to devise approaches to transform the series to obtain new series that is as stationary as possible. We will first perdorm and outlier analysis to make the series as stationary as possible. After that, we will decompose the series and build an ARIMA model on the series which we tried to transform into a stationary random series. In the end, we will compare the performances of different naive, ARIMA and another ARIMA models built with sliding window approach. Different performance metrics are used including WMAPE on a two week validation period.
...
```{r ana1, message = FALSE}
summary(ur.kpss(daily_mean_consuption$Consumption))
```

Summary of KPSS unit root test shows there is no significant evidence to reject the null hypothesis stating series is stationary. So although the test states the data might be stationary, value of test statistics is very close to critical values, so we should not be satisfied with that since we can observe an obvious seasonality.

```{r ana2, message = FALSE}
...
We have a pretty good model with adjusted r-squared value 0.9186, we can make use of this model to find those special days.

```{r out4, message = FALSE}
...
```{r dec2, message = FALSE}
print(ndiffs(mean_consumption_ts))
print(nsdiffs(mean_consumption_ts))
``` 

We need one seasonal differencing.

```{r dec3, message = FALSE}
...
Even though the naive base model is not improved significantly, both ARIMA model with components moving average and seasonal moving average and ARIMA with sliding window on electricity consumption data with seven day frequency worked fine. In order to make the series as stationary as possible, an outlier analysis is performed at first. If the outliers and the reasons of those special days were examined better, the final model could perform significantly better than naive model. The daily bias, daily weighted mean absolute percentage error for each date is reported in ARIMA with sliding window approach. In the end, the performance of naive and the other two ARIMA models are compared with their WMAPE.
